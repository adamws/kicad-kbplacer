...
- `layout2plot.py` - generate matplotlib plot with rectangles at each key position
...
$ hatch run tools:layout2plot {args...}
...
python tools/layout2plot.py {args...}
...
To generate matplotlib plot with rectangles at each key position:

```shell
hatch run tools:layout2plot --in examples/2x2/via.json --out output.png --width 0.8 --height 0.8
```

This creates a 2D plot showing rectangles of specified size (0.8 x 0.8 layout units) centered at each key position, with proper rotation applied.
