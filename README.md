...
<table>
  ...
  <tr>
    ...
    <td align="center" style="vertical-align: middle;">Keyboard Layout</td>
    <td>
      Layout file path or <a href="https://editor.keyboard-tools.xyz/">Keyboard Layout Editor NG</a>
      share link (<code>https://editor.keyboard-tools.xyz/#share=...</code>).
      Supports Keyboard Layout Editor NG
  ...
  <tr>
    ...
    <td>
      ...
      (defaults to <code>19.05</code>).</br>
      When selected layout file explicitly defines spacing in its metadata,
      these fields are updated with values from that file.
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>
      Diode position option with four possible choices:<br>
  ...
  <tr>
    <td align="center" style="vertical-align: middle;">Controller circuit template file</td>
    <td>
...
With empty `Keyboard Layout` field, switch placement is simply skipped.
...
Generated schematics are bundled into a KiCad project, i.e. `output.kicad_pro` is
created next to `output.kicad_sch`. The project basename is taken from `--pcb-file`
when given, otherwise from `--sch-file`.

It is also possible to generate an LED chain sheet (`--create-led-sch-file`, requires
KiCad 9.0 or above). When more than one sheet is requested, sheets are tied together
either as separate top-level sheets (`--bundle-strategy flat`, KiCad 10.0 and above)
or with a hierarchical root sheet (`--bundle-strategy hierarchical`). By default,
strategy is selected automatically based on KiCad version.
Run `python -m kbplacer --help` for details.
