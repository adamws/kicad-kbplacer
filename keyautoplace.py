...
class KeyPlacer():
    ...
    def AddTrackSegmentByPoints(self, start, stop, layer=B_Cu):
        ...
        layerName = self.board.GetLayerName(layer)
        self.logger.info(f"Adding track segment ({layerName}): [{start}, {stop}]")
        self.board.Add(track)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--route', help="Enable experimental routing")

    args = parser.parse_args()
    layoutPath = args.layout
    boardPath = args.board
    routeTracks = args.route is not None
